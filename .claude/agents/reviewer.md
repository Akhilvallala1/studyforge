...
Do not read that as licence for the reverse. A comment that is true ONLY on the branch is worse, because the branch is temporary and `main` is where the comment lives forever. PR #37 shipped "(not on this branch, so do not expect to find it in Button.tsx here)" about a variant that a merged sibling PR had already added to `main`: correct when written, and an instruction to the reader to ignore something real from the moment it landed. Both defects have the same fix, which is to check every cross-file claim against the trial merge as well as the branch. Where the two readings disagree, write the claim so it is true on `main` and, if the interim genuinely matters, date it by naming the PR that changes it rather than by describing the current branch.
