...
@pytest.mark.skip(reason="AC 10 at the prompt level: build_prompt is owned by ai-engineer")
def test_the_prompt_contains_no_answer_for_an_item_in_an_open_exposure():
    """Assert on the prompt TEXT, not on the reply.
    test_a_withheld_item_reaches_the_context_as_a_question_with_no_answer is the
    struct-level half and already passes."""


@pytest.mark.skip(reason="AC 11 at the prompt level: build_prompt is owned by ai-engineer")
def test_the_prompt_contains_the_answers_when_nothing_is_open():
    """Matching remediation.build_prompt.
    test_an_answer_the_learner_has_already_seen_is_present is the struct-level half."""


@pytest.mark.skip(reason="AC 19 at the prompt level: build_prompt is owned by ai-engineer")
def test_forged_material_markers_reach_the_tutor_prompt_neutralized():
    """The scrub itself is proved in test_untrusted.py against the same corpus. What is
    missing is a tutor prompt to run it through."""
